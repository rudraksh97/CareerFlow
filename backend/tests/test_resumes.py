import pytest
from datetime import datetime
from app.models.application import ApplicationStatus, ApplicationSource


class TestResumesAPI:
    """Test suite for resumes API endpoints"""

    def test_get_all_resumes_empty(self, client):
        """Test getting resumes when no applications exist"""
        response = client.get("/api/resumes/")
        
        assert response.status_code == 200
        result = response.json()
        assert len(result) == 0

    def test_get_all_resumes_with_data(self, client, create_test_application):
        """Test getting resumes with applications"""
        # Create applications with resumes
        app1 = create_test_application(
            company_name="Company A",
            job_title="Engineer A",
            resume_filename="resume_a.pdf"
        )
        app2 = create_test_application(
            company_name="Company B",
            job_title="Engineer B",
            resume_filename="resume_b.pdf"
        )
        
        response = client.get("/api/resumes/")
        
        assert response.status_code == 200
        result = response.json()
        assert len(result) == 2
        
        # Check that applications are ordered by date_applied desc
        assert result[0]["company_name"] in ["Company A", "Company B"]
        assert result[1]["company_name"] in ["Company A", "Company B"]
        assert result[0]["id"] != result[1]["id"]

    def test_get_all_resumes_ordering(self, client, create_test_application):
        """Test that resumes are ordered by date_applied desc"""
        from datetime import timedelta
        
        now = datetime.utcnow()
        
        # Create applications with different dates
        app1 = create_test_application(
            company_name="Old Company",
            job_title="Old Job",
            date_applied=now - timedelta(days=10)
        )
        app2 = create_test_application(
            company_name="New Company",
            job_title="New Job",
            date_applied=now
        )
        app3 = create_test_application(
            company_name="Middle Company",
            job_title="Middle Job",
            date_applied=now - timedelta(days=5)
        )
        
        response = client.get("/api/resumes/")
        
        assert response.status_code == 200
        result = response.json()
        assert len(result) == 3
        
        # Should be ordered by date_applied desc (newest first)
        assert result[0]["company_name"] == "New Company"
        assert result[1]["company_name"] == "Middle Company"
        assert result[2]["company_name"] == "Old Company"

    def test_get_all_resumes_includes_all_fields(self, client, create_test_application):
        """Test that resume endpoint returns all application fields"""
        app = create_test_application(
            company_name="Test Company",
            job_title="Test Job",
            job_id="test-123",
            job_url="https://example.com/job/123",
            portal_url="https://example.com/portal",
            status=ApplicationStatus.APPLIED,
            date_applied=datetime.utcnow(),
            email_used="test@example.com",
            resume_filename="test_resume.pdf",
            resume_file_path="uploads/resumes/test_resume.pdf",
            cover_letter_filename="test_cover.pdf",
            cover_letter_file_path="uploads/cover_letters/test_cover.pdf",
            source=ApplicationSource.LINKEDIN,
            notes="Test notes"
        )
        
        response = client.get("/api/resumes/")
        
        assert response.status_code == 200
        result = response.json()
        assert len(result) == 1
        
        resume_data = result[0]
        assert resume_data["id"] == app.id
        assert resume_data["company_name"] == "Test Company"
        assert resume_data["job_title"] == "Test Job"
        assert resume_data["job_id"] == "test-123"
        assert resume_data["job_url"] == "https://example.com/job/123"
        assert resume_data["portal_url"] == "https://example.com/portal"
        assert resume_data["status"] == ApplicationStatus.APPLIED.value
        assert resume_data["email_used"] == "test@example.com"
        assert resume_data["resume_filename"] == "test_resume.pdf"
        assert resume_data["resume_file_path"] == "uploads/resumes/test_resume.pdf"
        assert resume_data["cover_letter_filename"] == "test_cover.pdf"
        assert resume_data["cover_letter_file_path"] == "uploads/cover_letters/test_cover.pdf"
        assert resume_data["source"] == ApplicationSource.LINKEDIN.value
        assert resume_data["notes"] == "Test notes"
        assert "created_at" in resume_data
        assert "updated_at" in resume_data

    def test_get_all_resumes_with_different_statuses(self, client, create_test_application):
        """Test getting resumes with applications in different statuses"""
        # Create applications with different statuses
        app1 = create_test_application(
            company_name="Applied Company",
            status=ApplicationStatus.APPLIED
        )
        app2 = create_test_application(
            company_name="Interview Company",
            status=ApplicationStatus.INTERVIEW
        )
        app3 = create_test_application(
            company_name="Offer Company",
            status=ApplicationStatus.OFFER
        )
        app4 = create_test_application(
            company_name="Rejected Company",
            status=ApplicationStatus.REJECTED
        )
        
        response = client.get("/api/resumes/")
        
        assert response.status_code == 200
        result = response.json()
        assert len(result) == 4
        
        # All applications should be returned regardless of status
        companies = [app["company_name"] for app in result]
        assert "Applied Company" in companies
        assert "Interview Company" in companies
        assert "Offer Company" in companies
        assert "Rejected Company" in companies

    def test_get_all_resumes_with_different_sources(self, client, create_test_application):
        """Test getting resumes with applications from different sources"""
        # Create applications from different sources
        app1 = create_test_application(
            company_name="LinkedIn Company",
            source=ApplicationSource.LINKEDIN
        )
        app2 = create_test_application(
            company_name="Indeed Company",
            source=ApplicationSource.INDEED
        )
        app3 = create_test_application(
            company_name="Company Website",
            source=ApplicationSource.COMPANY_WEBSITE
        )
        
        response = client.get("/api/resumes/")
        
        assert response.status_code == 200
        result = response.json()
        assert len(result) == 3
        
        # All applications should be returned regardless of source
        companies = [app["company_name"] for app in result]
        assert "LinkedIn Company" in companies
        assert "Indeed Company" in companies
        assert "Company Website" in companies

    def test_get_all_resumes_with_cover_letters(self, client, create_test_application):
        """Test getting resumes with applications that have cover letters"""
        # Create applications with and without cover letters
        app1 = create_test_application(
            company_name="With Cover Letter",
            cover_letter_filename="cover1.pdf",
            cover_letter_file_path="uploads/cover_letters/cover1.pdf"
        )
        app2 = create_test_application(
            company_name="Without Cover Letter",
            cover_letter_filename=None,
            cover_letter_file_path=None
        )
        
        response = client.get("/api/resumes/")
        
        assert response.status_code == 200
        result = response.json()
        assert len(result) == 2
        
        # Find applications with and without cover letters
        with_cover = next(app for app in result if app["company_name"] == "With Cover Letter")
        without_cover = next(app for app in result if app["company_name"] == "Without Cover Letter")
        
        assert with_cover["cover_letter_filename"] == "cover1.pdf"
        assert with_cover["cover_letter_file_path"] == "uploads/cover_letters/cover1.pdf"
        assert without_cover["cover_letter_filename"] is None
        assert without_cover["cover_letter_file_path"] is None

    def test_get_all_resumes_with_notes(self, client, create_test_application):
        """Test getting resumes with applications that have notes"""
        # Create applications with and without notes
        app1 = create_test_application(
            company_name="With Notes",
            notes="This is a test note about the application"
        )
        app2 = create_test_application(
            company_name="Without Notes",
            notes=None
        )
        
        response = client.get("/api/resumes/")
        
        assert response.status_code == 200
        result = response.json()
        assert len(result) == 2
        
        # Find applications with and without notes
        with_notes = next(app for app in result if app["company_name"] == "With Notes")
        without_notes = next(app for app in result if app["company_name"] == "Without Notes")
        
        assert with_notes["notes"] == "This is a test note about the application"
        assert without_notes["notes"] is None

    def test_get_all_resumes_with_portal_urls(self, client, create_test_application):
        """Test getting resumes with applications that have portal URLs"""
        # Create applications with and without portal URLs
        app1 = create_test_application(
            company_name="With Portal",
            portal_url="https://company.com/careers/portal"
        )
        app2 = create_test_application(
            company_name="Without Portal",
            portal_url=None
        )
        
        response = client.get("/api/resumes/")
        
        assert response.status_code == 200
        result = response.json()
        assert len(result) == 2
        
        # Find applications with and without portal URLs
        with_portal = next(app for app in result if app["company_name"] == "With Portal")
        without_portal = next(app for app in result if app["company_name"] == "Without Portal")
        
        assert with_portal["portal_url"] == "https://company.com/careers/portal"
        assert without_portal["portal_url"] is None

    def test_get_all_resumes_with_different_emails(self, client, create_test_application):
        """Test getting resumes with applications using different emails"""
        # Create applications with different emails
        app1 = create_test_application(
            company_name="Company A",
            email_used="personal@example.com"
        )
        app2 = create_test_application(
            company_name="Company B",
            email_used="work@example.com"
        )
        app3 = create_test_application(
            company_name="Company C",
            email_used="career@example.com"
        )
        
        response = client.get("/api/resumes/")
        
        assert response.status_code == 200
        result = response.json()
        assert len(result) == 3
        
        # All applications should be returned with their respective emails
        emails = [app["email_used"] for app in result]
        assert "personal@example.com" in emails
        assert "work@example.com" in emails
        assert "career@example.com" in emails

    def test_get_all_resumes_with_different_resume_filenames(self, client, create_test_application):
        """Test getting resumes with different resume filenames"""
        # Create applications with different resume filenames
        app1 = create_test_application(
            company_name="Company A",
            resume_filename="resume_v1.pdf"
        )
        app2 = create_test_application(
            company_name="Company B",
            resume_filename="resume_v2.pdf"
        )
        app3 = create_test_application(
            company_name="Company C",
            resume_filename="my_resume.docx"
        )
        
        response = client.get("/api/resumes/")
        
        assert response.status_code == 200
        result = response.json()
        assert len(result) == 3
        
        # All applications should be returned with their respective resume filenames
        filenames = [app["resume_filename"] for app in result]
        assert "resume_v1.pdf" in filenames
        assert "resume_v2.pdf" in filenames
        assert "my_resume.docx" in filenames

    def test_get_all_resumes_with_different_resume_paths(self, client, create_test_application):
        """Test getting resumes with different resume file paths"""
        # Create applications with different resume file paths
        app1 = create_test_application(
            company_name="Company A",
            resume_file_path="uploads/resumes/company_a_resume.pdf"
        )
        app2 = create_test_application(
            company_name="Company B",
            resume_file_path="uploads/resumes/company_b_resume.pdf"
        )
        
        response = client.get("/api/resumes/")
        
        assert response.status_code == 200
        result = response.json()
        assert len(result) == 2
        
        # All applications should be returned with their respective resume file paths
        paths = [app["resume_file_path"] for app in result]
        assert "uploads/resumes/company_a_resume.pdf" in paths
        assert "uploads/resumes/company_b_resume.pdf" in paths

    def test_get_all_resumes_with_special_characters(self, client, create_test_application):
        """Test getting resumes with special characters in company names and job titles"""
        # Create applications with special characters
        app1 = create_test_application(
            company_name="Tech & Co. (Inc.)",
            job_title="Senior Software Engineer - Full Stack"
        )
        app2 = create_test_application(
            company_name="Startup.io",
            job_title="DevOps Engineer (AWS/GCP)"
        )
        
        response = client.get("/api/resumes/")
        
        assert response.status_code == 200
        result = response.json()
        assert len(result) == 2
        
        # All applications should be returned with special characters preserved
        companies = [app["company_name"] for app in result]
        titles = [app["job_title"] for app in result]
        
        assert "Tech & Co. (Inc.)" in companies
        assert "Startup.io" in companies
        assert "Senior Software Engineer - Full Stack" in titles
        assert "DevOps Engineer (AWS/GCP)" in titles

    def test_get_all_resumes_with_unicode_characters(self, client, create_test_application):
        """Test getting resumes with unicode characters"""
        # Create applications with unicode characters
        app1 = create_test_application(
            company_name="Café & Co.",
            job_title="Développeur Senior"
        )
        app2 = create_test_application(
            company_name="Tech Solutions 🚀",
            job_title="Software Engineer 💻"
        )
        
        response = client.get("/api/resumes/")
        
        assert response.status_code == 200
        result = response.json()
        assert len(result) == 2
        
        # All applications should be returned with unicode characters preserved
        companies = [app["company_name"] for app in result]
        titles = [app["job_title"] for app in result]
        
        assert "Café & Co." in companies
        assert "Tech Solutions 🚀" in companies
        assert "Développeur Senior" in titles
        assert "Software Engineer 💻" in titles

    def test_get_all_resumes_with_long_text(self, client, create_test_application):
        """Test getting resumes with long text in fields"""
        # Create applications with long text
        long_company = "A" * 100
        long_title = "B" * 200
        long_notes = "C" * 500
        
        app1 = create_test_application(
            company_name=long_company,
            job_title=long_title,
            notes=long_notes
        )
        
        response = client.get("/api/resumes/")
        
        assert response.status_code == 200
        result = response.json()
        assert len(result) == 1
        
        # Long text should be preserved
        assert result[0]["company_name"] == long_company
        assert result[0]["job_title"] == long_title
        assert result[0]["notes"] == long_notes

    def test_get_all_resumes_with_newlines_in_notes(self, client, create_test_application):
        """Test getting resumes with newlines in notes"""
        # Create application with newlines in notes
        notes_with_newlines = "First line\nSecond line\nThird line"
        
        app = create_test_application(
            company_name="Test Company",
            notes=notes_with_newlines
        )
        
        response = client.get("/api/resumes/")
        
        assert response.status_code == 200
        result = response.json()
        assert len(result) == 1
        
        # Newlines should be preserved
        assert result[0]["notes"] == notes_with_newlines

    def test_get_all_resumes_with_tabs_in_notes(self, client, create_test_application):
        """Test getting resumes with tabs in notes"""
        # Create application with tabs in notes
        notes_with_tabs = "Field1\tField2\tField3"
        
        app = create_test_application(
            company_name="Test Company",
            notes=notes_with_tabs
        )
        
        response = client.get("/api/resumes/")
        
        assert response.status_code == 200
        result = response.json()
        assert len(result) == 1
        
        # Tabs should be preserved
        assert result[0]["notes"] == notes_with_tabs 