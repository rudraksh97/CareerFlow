  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="h-5 bg-neutral-200 rounded w-80 animate-pulse"></div>
      </div>
...
function MetricCard({ title, value, icon: Icon, color, bgColor }: { title: string; value: string | number; icon: React.ElementType; color: string; bgColor: string }) {
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm font-medium text-neutral-600 mb-1 group-hover:text-neutral-700 transition-colors duration-200">{title}</p>
          <p className="text-2xl font-bold text-neutral-900 group-hover:text-blue-600 transition-colors duration-200">{value}</p>
        </div>
        <div className={`p-3 rounded-xl ${bgColor} border transition-all duration-200 group-hover:scale-105`}>
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-neutral-600 mb-4">There was an error loading your analytics. Please try again.</p>
          <button 
    ...
    >
      ...
      {!hasData ? (
        ...
        >
          ...
          <p className="text-neutral-500 max-w-md">Add applications to see analytics and insights here. Track your progress, success rates, and identify the most effective job sources.</p>
        </motion.div>
      ) : (
        <>
          {/* Dashboard Overview */}
          ...
          <motion.div variants={itemVariants} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            {dashboardData?.applications_by_status && (
              ...
              >
                ...
                <div className="space-y-4">
                  {Object.entries(dashboardData.applications_by_status).map(([status, count], index) => (
                    <motion.div 
                      key={status} 
