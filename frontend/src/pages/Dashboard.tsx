      ...
      <motion.div variants={itemVariants} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <motion.div variants={itemVariants} className="card p-6 lg:col-span-2">
          <div className="flex items-center justify-between mb-6">
            ...
            <Link to="/applications" className="text-sm font-semibold text-primary-600 hover:text-primary-700">
              View all
            </Link>
          </div>
          <div className="space-y-3">
            {activityLoading ? (
              [...Array(3)].map((_, i) => (
                <motion.div variants={itemVariants} key={i} className="flex items-center p-4 rounded-xl bg-neutral-50 border border-neutral-200/80 animate-pulse">
                  <div className="w-8 h-8 bg-neutral-200 rounded-lg mr-4"></div>
              ...
              recentActivity.map(app => (
                <Link 
                  to={`/applications?q=${app.id}`} 
                  key={app.id}
                  className="group flex items-center p-4 rounded-xl bg-neutral-50 border border-neutral-200/80 hover:border-primary-300 hover:bg-primary-50 transition-all"
                >
                  <div className="p-2.5 rounded-lg bg-primary-50 border border-primary-200/80 mr-4">
                    <Building className="h-4 w-4 text-primary-600" />
                  </div>
                  <div className="flex-1">
                    <p className="font-semibold text-neutral-800">{app.company_name}</p>
                    <p className="text-sm text-neutral-500">{app.job_title}</p>
                  </div>
              ...
              <motion.div variants={itemVariants} className="flex flex-col items-center justify-center p-8 rounded-xl bg-neutral-50 border border-neutral-200/80 text-center">
                <div className="p-3 rounded-full bg-primary-100 border-8 border-primary-50 mb-4">
                  <Activity className="h-6 w-6 text-primary-600" />
                </div>
                <h3 className="font-semibold text-neutral-800">No recent activity</h3>
                <p className="text-sm text-neutral-500 mt-1">Your latest job applications will appear here.</p>
              </motion.div>
            )}
          </div>
        </motion.div>
        ...
        <motion.div variants={itemVariants} className="card p-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {Object.entries(analytics.applications_by_status).map(([status, count]) => (
              <motion.div variants={itemVariants} key={status} className="text-center p-4 rounded-xl bg-neutral-50 border border-neutral-200">
